...
public class ChatController {
    ...
    @Autowired(required = false)  // MCP 禁用时可为空
    ...
    /**
     * 专用 RAG 流式接口。
     * 直接走 QueryRewriteService -> HybridSearchService -> RagService 生成链路，便于验证知识库问答质量。
     */
            ...
            try {
                ragService.queryStream(request.getQuestion(), history, new RagService.StreamCallback() {
                    ...
                    public void onSearchResults(List<VectorSearchService.SearchResult> results) {
                        logger.info("RAG 检索完成 - SessionId: {}, results: {}", request.getId(), results.size());
                    }

                    @Override
                    public void onReasoningChunk(String chunk) {
                        // 当前模型未单独拆分 reasoning，保留接口兼容。
                    }
                    ...
                    public void onComplete(String fullContent, String fullReasoning) {
                        try {
                            String answer = fullContent != null && !fullContent.isEmpty()
                                    ? fullContent
                                    : fullAnswerBuilder.toString();
                    ...
                    public void onError(Exception e) {
                        try {
                            ...
                            logger.error("发送 RAG 错误消息失败", ex);
                        }
                        emitter.completeWithError(e);
                    }
                });
            } catch (Exception e) {
                logger.error("RAG 流式接口失败", e);
                try {
                    emitter.send(SseEmitter.event()
                            .name("message")
                            .data(SseMessage.error(e.getMessage()), MediaType.APPLICATION_JSON));
                } catch (IOException ex) {
                    logger.error("发送 RAG 初始化错误消息失败", ex);
                }
                emitter.completeWithError(e);
            }
        });
    ...
    public SseEmitter aiOps() {
        ...
        executor.execute(() -> {
            try {
                ...
                ToolCallback[] toolCallbacks = (!clsMockEnabled && tools != null)
