...
public class VectorIndexService {
    ...
    public void indexSingleFile(String filePath) throws Exception {
        ...
        // 2.1 同步删除 BM25 索引中的旧数据
        deleteBM25Data(path.toString());
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            try {
                ...
                // 生成文档 ID（与 insertToMilvus 中保持一致）
                ...
                // 插入到 Milvus
                insertToMilvus(docId, chunk.getContent(), vector, metadata);

                // 同步更新 BM25 索引
                bm25SearchService.addDocument(docId, chunk.getContent(),
    ...
    /**
     * 删除 BM25 索引中的旧数据（根据文件路径前缀匹配）
     */
    private void deleteBM25Data(String filePath) {
        try {
            String normalizedPath = Paths.get(filePath).normalize().toString().replace("\\", "/");
            int removed = bm25SearchService.removeDocumentsBySource(normalizedPath);
            logger.info("BM25 索引清理完成: {}, 清理 {} 条记录", normalizedPath, removed);
        } catch (Exception e) {
            logger.warn("BM25 索引清理失败（可能是首次索引）: {}", e.getMessage());
        }
    }
     ...
     * 插入向量到 Milvus
     */
    private void insertToMilvus(String docId, String content, List<Float> vector, 
        ...
        try {
            ...
            logger.debug("向量插入成功: id={}, source={}", docId, metadata.get("_source"));
